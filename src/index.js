...
const protectedWorkbooksConfig = require('./config/protected-workbooks.json')

const PROTECTED_WORKBOOK_NAMES = new Set(
  protectedWorkbooksConfig.names.map((name) => name.toLowerCase()),
)
...
ipcMain.handle('populate-files-tab', async (_event, folderPath, rootFolder) => {
  ...
  const files = allEntries.filter(
    (e) =>
      ...
      !e.name.startsWith('.') &&
      !e.name.startsWith('~$') &&
      !PROTECTED_WORKBOOK_NAMES.has(e.name.toLowerCase()),
  )
